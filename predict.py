...
st.write("""
         您可以在下方选择模型并上传csv文件进行测试，csv文件请按电池的个数按行处理，即假设如果有4个电池，每个电池的电池容量序列长度为168，则csv文件的前四行应为各个电池的电池容量数据，形成（4,168）矩阵的形式。
         
         注意下方只提供我们使用NASA和CALCE数据集训练好的模型，如果要用您自己的数据集进行训练，可以在train页面重新训练一个模型，之后再回到本页面选中您的模型再上传全部数据，本页面会显示包括训练集和测试集全部数据的预测结果方便您观察。
         """)
# 创建一个选择框，让用户选择模型权重文件
option = st.selectbox(
        '请选择模型权重文件',
